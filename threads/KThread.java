...
public class KThread {
	...
	//	System.out.print("Here in finish");
	
	currentThread.status = statusFinished;
	joinLock.acquire();
	//System.out.println("Here in finish");

	while(currentThread.waitQueue.nextThread()!= null)
	{
		//System.out.println("Here in finish while");
		currentThread.waitQueue.acquire(currentThread.waitQueue.nextThread());
		//currentThread.monitor.wake();
	}
	currentThread.monitor.wakeAll();
    ...
    private Condition monitor = new Condition(joinLock);
    public void join() {
    	Lib.debug(dbgThread, "Joining to thread: " + toString());
    	joinLock.acquire();
    	Machine.interrupt().disable();
    	System.out.println("Here in join general");
    	Lib.assertTrue(this != currentThread);
    	
    	if(this.status == statusFinished)
    	{
    		System.out.println("Here in join if");
    		joinLock.release();	
    		//return;
    	}
    	else
    	{	
    		counting++;
//    		System.out.println("Here in join else");	
    		...
    		if(counting >0)
    		{
   			System.out.println("Here in join else again");
    		}
    		joinLock.release();	
    		//return;
    	}	
...
//    Provided test joinTest2.java
    ...
    */
     ...
     * on the ready queue and not running). //monitor.sleep();
     */
